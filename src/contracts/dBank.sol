...
contract dBank {
  ...
  function withdraw() public {
    ...
    // send funds to user
    msg.sender.transfer(etherBalanceOf[msg.sender]+interest); // eth back to user
  ...
  function payOff() public {
    ...
    require(token.transferFrom(msg.sender, address(this), collateralEther[msg.sender] / 2), "Error, can't receive tokens"); // must approve dBank 1st
        ...
        // require(token.transferFrom(address(this), msg.sender, collateralEther[msg.sender] / 2), "Error, can't receive tokens"); // must approve dBank 1st    uint fee = collateralEther[msg.sender] / 10; // calc 10% fee
